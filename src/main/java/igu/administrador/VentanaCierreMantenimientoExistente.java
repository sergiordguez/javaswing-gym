...
public class VentanaCierreMantenimientoExistente extends JDialog {
	 ...
	 * Create the frame.
	 */
	public VentanaCierreMantenimientoExistente(VentanaConfirmacionMantenimiento va) {
		setResizable(false);
	...
	private JLabel getLblFechaInicio() {
		if (lblFechaInicio == null) {
			lblFechaInicio = new JLabel("Fecha fin:");
			lblFechaInicio.setBounds(293, 114, 141, 28);
		}
	...
	private JTextField getTxtFechaInicio() {
		if (txtFechaInicio == null) {
			...
			txtFechaInicio.setBounds(293, 153, 254, 34);
			...
			btnCerrar.addActionListener(new ActionListener() {
				...
				public void actionPerformed(ActionEvent e) {
					if (getTxtFechaInicio().getText().isEmpty()) {
						...
						cerrarInstalacion();
						JOptionPane.showMessageDialog(null, "Añadida hora fin al mantenimiento");
					}
				}
			});
	...
	private JButton getBtnAñadirHora() {
		if (btnAñadirHora == null) {
			btnAñadirHora = new JButton("Añadir Hora Fin");
	...
	private void repintarHoras() {
		DefaultListModel<String> modelo = new DefaultListModel<>();
		List<Integer> hours = new ArrayList<>();
		int horaInicio = ac.listarMantenimiento((InstalacionDto) getCbInstalaciones().getSelectedItem())
				.getFechaInicio().getHours();
	...
	private void cerrarInstalacion() {
		ac.modificarHoraMantenimiento(horaInicio.date);
	}
	...
	private JButton getBtnEliminarInicio() {
		if (btnEliminarInicio == null) {
			btnEliminarInicio = new JButton("Cancelar fin");
	...
	private JLabel getLblInstalacion() {
		if (lblInstalacion == null) {
			...
			lblInstalacion.setBounds(293, 33, 141, 28);
		}
	...
	private JComboBox<InstalacionDto> getCbInstalaciones() {
		if (cbInstalaciones == null) {
			...
			cbInstalaciones.setBounds(378, 33, 169, 28);
		}
	...
	private void rellenarCombo() {
		...
		ActividadDto[] inst = ac.listarInstalacionesMantenimiento();
		for (ActividadDto element : inst) {
			InstalacionDto dto = new InstalacionDto();
			dto.setNombre(element.getInstalacion());
